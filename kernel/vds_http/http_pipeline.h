...
namespace vds {
  ...
  private:
    ...
    std::future<void> send( const vds::http_message & message);
    std::future<void> continue_send();
  };

  inline std::future<void> http_pipeline::continue_read_data() {
  ...
  inline std::future<void> http_pipeline::finish_message() {
    co_return;
  }  
}
