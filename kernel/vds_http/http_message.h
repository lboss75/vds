...
namespace vds {
  ...
  public:
    ...
    : headers_(headers), body_(std::make_shared<continuous_stream<uint8_t>>())
    {
    }
    ...
    std::shared_ptr<continuous_stream<uint8_t>> & body() {
      return this->body_;
    }

  private:
    std::list<std::string> headers_;
    std::shared_ptr<continuous_stream<uint8_t>> body_;
  };
}
