...
namespace vds {
  ...
  public:
    ...
    http_route_handler(
      ...
      const std::function<async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>>(
        ...
        const std::shared_ptr<http_async_serializer> & output_stream,
        const http_message &)> & callback)
      : url_(url), method_(method), handler_(new api_base_handler(callback)) {
    }
    ...
    http_route_handler(
      ...
      const std::function<async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>>(
        ...
        const std::shared_ptr<user_manager> &,
        const http_message &)> & callback)
      : url_(url), method_(method), handler_(new api_auth_base_handler(callback)) {
    }
  ...
  private:
    ...
    public:
      ...
      virtual async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>> process(
        const service_provider * sp,
        const std::shared_ptr<http_async_serializer> & output_stream,
        const http_router * router,
    ...
    class api_base_handler : public handler_base {
    public:
      api_base_handler(
        const std::function<async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>>(
          const service_provider *,
      ...
      handler_base * clone() override {
        return new api_base_handler(*this);
      }
    ...
    private:
      std::function<async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>>(
        const service_provider *,
        const std::shared_ptr<http_async_serializer> &,
        const http_message &)> callback_;
    };

    class api_auth_base_handler : public handler_base {
    public:
      api_auth_base_handler(
        const std::function<async_task<expected<std::shared_ptr<stream_output_async<uint8_t>>>>(
          const service_provider *,
          const std::shared_ptr<http_async_serializer> & output_stream,
          const std::shared_ptr<user_manager> &,
      ...
      handler_base * clone() override {
        return new api_auth_base_handler(*this);
      }
