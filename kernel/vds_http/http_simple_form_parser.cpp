  ...
  if (message.get_header("Content-Type", content_type)) {
    ...
    if (multipart_form_data == content_type.substr(0, sizeof(multipart_form_data) - 1)) {
      ...
      if (form_urlencoded == content_type.substr(0, sizeof(form_urlencoded) - 1)) {
        auto task = std::make_shared<form_parser>(this->shared_from_this());
        co_return co_await task->read_form_urlencoded(sp, message);
      }
      else {
        throw std::runtime_error("Invalid content type " + content_type);
      }
...
std::future<std::string> vds::http::simple_form_parser::form_parser::read_string_body(
  ...
  std::string buffer;
  for (;;) {
    size_t readed = co_await part.body()->read_async(sp, this->buffer_, sizeof(this->buffer_));
    if (0 == readed) {
      co_return buffer;
    }
    ...
    if (part.get_header("Content-Disposition", content_disposition)) {
      ...
      if (!items.empty() && "form-data" == *items.begin()) {
        ...
        if (values.end() != pname) {
          ...
          auto buffer = co_await this->read_string_body(sp, part);
          this->owner_->values_[name] = buffer;
        }

        co_return co_await this->skip_part(sp, part);
      }
    }
