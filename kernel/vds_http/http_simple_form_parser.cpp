...
//
//vds::async_task<vds::expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>> vds::http::simple_form_parser::parse(
//	const service_provider * sp,
//	const http_message& message,
//	lambda_holder_t<async_task<expected<void>>, const std::shared_ptr<http::simple_form_parser> &> handler) {
//
//	auto parser = std::make_shared<simple_form_parser>();
//
...
//					sp,
//					"--" + boundary,
//					[parser](http_message && part)->vds::async_task<vds::expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>> {
//						return read_part(parser, part);
//					},
//          [h = std::move(handler), parser]() {
//            return h(parser);
//          });
//			}
//			else {
...
//}
//
//vds::async_task<vds::expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>> vds::http::simple_form_parser::read_string_body(
...
//}
//
//vds::async_task<vds::expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>> vds::http::simple_form_parser::read_part(
...
//				return read_string_body([owner, name](std::string && value) {
//					owner->values_[name] = value;
...
//}
//
//vds::async_task<vds::expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>> vds::http::simple_form_parser::read_form_urlencoded(
//	const std::shared_ptr<simple_form_parser> & owner,
//	const http_message & /*part*/,
//  lambda_holder_t<vds::async_task<vds::expected<void>>> final_handler) {
//
//	return read_string_body([owner, h = std::move(final_handler)](std::string && buffer) {
...
//				}
//
//				owner->values_[item.substr(0, p)] = url_encode::decode(value);
//			}
//		}
//
//		return h();
