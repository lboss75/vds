...
{
  sp.get<logger>()->trace("server_log", sp, "Broadcast %s:%s", record.id().str().c_str(), record.serialize()->str().c_str());
  ...
  const server_log_record_broadcast & message)
{
  ...
  if((*manager)->save_record(
    ...
    message.record(),
    message.signature())){
    sp.get<logger>()->debug("server_log", sp, "Got %s", message.record().id().str().c_str());
  ...
  for (auto p : message.unknown_records()) {
    ...
    const_data_buffer signature;
      if((*sp.get<principal_manager>())->get_record(sp, tr, p, record, signature)) {
        sp.get<logger>()->debug("server_log", sp, "Provided %s", record.id().str().c_str());
  ...
  database_transaction & tr)
{
  ...
  for (auto& p : unknown_records) {
    sp.get<logger>()->debug("server_log", sp, "Require %s", p.str().c_str());
  }
