...
vds::async_task<vds::client_messages::task_state> vds::ilocal_cache::download_object(
  const service_provider & sp,
...
vds::async_task<vds::client_messages::task_state> vds::_local_cache::download_object(
  const service_provider & sp,
  const guid & version_id,
  const filename & result_data)
{
  return [sp, version_id, result_data](
    const async_result<client_messages::task_state> & result) {
    (*sp.get<iserver_database>())->get_db()->async_transaction(sp,
      [sp, version_id, result_data, result](database_transaction & tr){
        ...
        if (total_data != downloaded_data) {
          result.done(client_messages::task_state{
            client_messages::task_state::task_status::IN_PROGRESS,
            "Downloading data...",
          ...
          result.done(client_messages::task_state{
            client_messages::task_state::task_status::DONE,
            "Done",
            100/*,
            std::chrono::system_clock::now(),
            std::chrono::system_clock::now()*/
