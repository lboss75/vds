...
: dht_user_id_(dht_user_id),
  ...
  login_state_(login_state_t::waiting_channel){
}

void vds::security_walker::update(
  ...
  while (st.execute()) {
    ...
    case transactions::transaction_block::block_type_t::self_signed: {
      if(this->user_password_hash_ != crypted_key) {
        this->login_state_ = login_state_t::login_failed;
      }
      else {
    ...
    while (0 < s.size()) {
      ...
      case transactions::transaction_id::root_user_transaction: {
        if(this->login_state_ != login_state_t::waiting_channel) {
        ...
        this->login_state_ = login_state_t::login_sucessful;
        break;
      }
