...
vds::async_task<std::shared_ptr<vds::json_value>> vds::user_storage::device_storages(
...
std::shared_ptr<vds::json_value> vds::user_storage::device_storage_label(
  const std::shared_ptr<user_manager>& user_mng) {
  ...
  auto body = result->json_value::str();
...
vds::async_task<void> vds::user_storage::add_device_storage(const service_provider* sp,
...
vds::async_task<bool> vds::user_storage::local_storage_exists(const service_provider* sp) {

  auto result = std::make_shared<bool>();

  co_await sp->get<db_model>()->async_read_transaction([sp, result](database_read_transaction & t) {
    auto client = sp->get<dht::network::client>();
    auto current_node = client->current_node_id();
