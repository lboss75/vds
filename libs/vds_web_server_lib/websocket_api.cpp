  ...
  const http_message & request)
{
  return websocket::open_connection(
    ...
    return expected<std::shared_ptr<vds::stream_output_async<uint8_t>>>(std::make_shared<collect_data>([sp, output_stream](const_data_buffer data)->async_task<expected<void>> {
