...
{
  sp.get<logger>()->debug("route", sp, "Route message %d to %s", message.msg_type_id(), message.target_server_id().str().c_str());
  ...
  if (sp.get<istorage_log>()->current_server_id() == message.target_server_id()) {
    ...
    sp.get<logger>()->debug("route", sp, "Route message %d", message.msg_type_id());
